...
- `snomed-ct.json`: `concepts` and `relationships` (`isA`, `findingSite`).
...
The SNOMED snapshot uses the nine catalog Joint concepts and their depth-three descendant
closures. It also includes the four authored-condition ClinicalFinding concepts and their
`findingSite` AnatomicalStructure concepts. The script stops if EVS changes the subset far
outside its expected size or returns mixed terminology versions, so a refresh receives
human review instead of silently changing the graph.
