...
JOINT_CODES = {
    ...
    "hip": "24136001",
    "knee": "49076000",
    "lumbar spine": "52612000",
    "shoulder": "16982005",
    "thoracic spine": "786965005",
    "wrist": "74670003",
}

ANATOMY_SUBSET_ROOT_CODES = {
    **JOINT_CODES,
...
def fetch_concept(client: AcquisitionClient, code: str) -> dict[str, Any]:
    payload = client.json(
...
def fetch_concepts(
    client: AcquisitionClient, codes: set[str], workers: int
...
def acquire_snomed(
    client: AcquisitionClient, retrieved_at: str, workers: int
) -> dict[str, Any]:
    descendant_codes: set[str] = set()
    for code in ANATOMY_SUBSET_ROOT_CODES.values():
    ...
    initial_codes = (
        set(JOINT_CODES.values())
        | set(ANATOMY_SUBSET_ROOT_CODES.values())
        | set(CONDITION_CODES.values())
    )
    initial_payloads = fetch_concepts(client, initial_codes, workers)

    finding_sites_by_condition = {
        code: {
    ...
    missing_finding_sites = [
        code for code, site_codes in finding_sites_by_condition.items() if not site_codes
    ]
    if missing_finding_sites:
        raise AcquireError(
            "each authored condition must have a findingSite edge; missing for "
            + ", ".join(sorted(missing_finding_sites, key=int))
        )
    finding_site_codes = set().union(*finding_sites_by_condition.values())
    ...
    for code in sorted(anatomy_codes, key=int):
        for parent in payloads[code].get("parents") or []:
            ...
            if parent_code not in anatomy_codes or not has_qualifier(parent, "isa"):
                continue
            edge_id = f"snomedct:{code}:isA:{parent_code}"
            edges_by_id[edge_id] = {
                "id": edge_id,
            ...
            parents_by_code.setdefault(code, set()).add(parent_code)
    ...
    for code in sorted(CONDITION_CODES.values(), key=int):
        for association in payloads[code].get("associations") or []:
            ...
            edge_id = f"snomedct:{code}:findingSite:{target_code}"
            edges_by_id[edge_id] = {
                "id": edge_id,
                "source_id": concept_id(code),
                "type": "findingSite",
    ...
    joint_codes = set(JOINT_CODES.values())
    ungrounded_conditions = []
    for code, site_codes in finding_sites_by_condition.items():
        seen: set[str] = set()
        frontier = set(site_codes)
        while frontier and not frontier.intersection(joint_codes):
            seen.update(frontier)
            frontier = {
                parent_code
                for child_code in frontier
                for parent_code in parents_by_code.get(child_code, set())
                if parent_code not in seen
            }
        if not frontier.intersection(joint_codes):
            ungrounded_conditions.append(code)
    if ungrounded_conditions:
        raise AcquireError(
            "each authored condition must reach a Joint through findingSite and isA; "
            "missing for " + ", ".join(sorted(ungrounded_conditions, key=int))
        )
        ...
        "scope_roots": [
            {
                "id": concept_id(code),
                "catalog_term": term,
                "subset_root_id": concept_id(ANATOMY_SUBSET_ROOT_CODES[term]),
...
def acquire_copper(client: AcquisitionClient, retrieved_at: str) -> dict[str, Any]:
...
def build_artifacts(
    ...
    snomed = acquire_snomed(client, retrieved_at, workers)
...
def main() -> None:
    ...
    client = HttpClient()
    artifacts = build_artifacts(client, retrieved_at, args.workers)
    write_artifacts(args.output_dir, artifacts)

    snomed = artifacts["snomed-ct.json"]
    copper = artifacts["copper.json"]
    anatomy_count = sum(
